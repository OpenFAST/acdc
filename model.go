...
// Model structure contains slices of all file types
...
// NewModel returns the Model structure with all slices initialized to empty
func NewModel() *Model {
	return &Model{
		Main:        []Main{},
		ElastoDyn:   []ElastoDyn{},
...
func (m *Model) Add(fileType string) any {
...
func ParseModel(MainPath string) (*Model, error) {
	...
	model.Main = []Main{{}}
	if err := model.parseFile(MainPath, &model.Main[0]); err != nil {
		return nil, err
	}
...
func (m *Model) parseFile(path string, s any) error {
	...
	for i := 1; i < sVal.NumField(); i++ {
		...
		if p, ok := fieldVal.Addr().Interface().(*Path); ok {
			...
			if name, ok := m.PathMap[subpath]; ok {
				p.Value = name
				continue
			}
			...
			if p.Root {
				...
				for _, match := range matches {
					...
					if err := m.parseFile(match, ss); err != nil {
				...
				// Parse file
				if err := m.parseFile(subpath, ss); err != nil {
					return fmt.Errorf("error parsing '%s': %w", subpath, err)
				}
			}
...
func (m *Model) Write(dir string) error {
